...
@Component({
  selector: 'daily-stocks',
  templateUrl: './daily-stocks.component.html',
  styleUrls: ['./daily-stocks.component.css']
})
export class DailyStocksComponent implements OnInit {
  ...
  options = {
    title: {
      text: `Daily Stocks of ${this.selectedCompany}`
    },
    rangeSelector: {
      selected: 4
    },
  ...
  ngOnInit(): void {
    this._stocksService.getDailyStockSeries(this.selectedCompany).subscribe(
      stocksData => {
        this.dailyStocks = formatChartData(stocksData["Time Series (Daily)"]);
        this.closingPrice = this.dailyStocks[this.dailyStocks.length - 1][4];
        this.options.series[0].data = this.dailyStocks;
      }
    )
  }
  onValueChange(e): void {
    ...
    this._stocksService.getDailyStockSeries(this.selectedCompany).subscribe(
      stocksData => {
        this.dailyStocks = formatChartData(stocksData["Time Series (Daily)"]);
        if (this.dailyStocks.length > 0) {
          this.closingPrice = this.dailyStocks[this.dailyStocks.length - 1][4];
          this.options.title.text = `Daily Stocks of ${this.selectedCompany}`;
